      ...
      {mode === 'list' && (
        <>
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <StatCard
              label="Under Review"
              value={apps.filter((a) => a.stepPhase === 2 || a.status === 'Under Verification').length}
            ...
            <StatCard
              label="In Processing"
              value={apps.filter((a) => a.stepPhase >= 3 && a.stepPhase <= 6).length}
              sub="Processing stages"
              icon={ShieldCheck}
              variant="blue"
            />
            <StatCard
              label="Completed Requests"
              value={apps.filter((a) => a.stepPhase >= 7 || a.status === 'Completed' || a.status === 'Approved').length}
