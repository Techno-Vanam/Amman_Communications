...
export default function PortalDashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 font-sans pb-2">
      {/* 4 KPI Summary Cards Grid - Perfectly Balanced h-36 Height */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1: Active Applications */}
        <div className="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-2xs relative flex flex-col justify-between h-36">
          <div>
            ...
            <p className="text-3xl font-extrabold text-[#0e2a47] tracking-tight mt-2.5">
              {applications.length}
            </p>
          </div>
          <div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-2xs relative flex flex-col justify-between h-36">
          ...
          <div>
            <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-800 border border-blue-200/60">
              Total completed
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 p-5 shadow-2xs relative flex flex-col justify-between h-36">
          ...
          <div>
            <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-bold bg-indigo-50 text-indigo-800 border border-indigo-200/60">
              Booked sessions
            </span>
          </div>
        </div>
            ...
            <div className="mt-3 space-y-2.5">
              {recentActivities.length === 0 ? (
                ...
                recentActivities.map((act) => (
                  <div key={act.id} className="p-3.5 rounded-xl bg-gray-50/80 border border-gray-200/70 flex items-start gap-3.5 transition-all hover:bg-gray-50">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-gray-900 font-bold leading-snug">
                        {act.title} &mdash; <span className="text-[#12372A]">{act.service}</span>
                      </p>
                      <div className="flex items-center justify-between text-xs text-gray-500 mt-1">
                        <span className="font-semibold text-gray-700 bg-gray-200/60 px-2 py-0.5 rounded-md text-[10px]">
                          {act.status}
                        </span>
